...
class MapCreator():
    ...
    def Create_Map(self,location):
        ...
        self.prev_drawing = 0
        self.mapObj = folium.Map(location=location,
							zoom_start=self.zoom)# Create map object
        self.geodesic = Geod(ellps='WGS84')
        self.create_polygon()
			
					

        self.mapObj.save('output.html')# Save the Html map object

    def UpdateMap(self,drawing_initialized,enemy_list):
        self_telemetry = self.widget_update(drawing_initialized,enemy_list)
        self.mapObj.save('output.html')# Save the Html map object
        return self_telemetry
    def widget_update(self,drawing_initialized,enemy_list):
            global start2 
            ...
            if enemy_list != 0 and enemy_prev_list!= 0 and enemy_prev_list != []:
                ...
                if condition:
                    ...
                    if self.prev_drawing == 1:
                        self.Create_Map(self.coordinate)
                        self.draw_flag = True
                        self.prev_drawing = 0
                    ...
                    for a in range(len(enemy)):
                        ...
                        rotation = 10*math.atan((coordinates[1]-coordinates_prev[1]-0.00000000001)/(coordinates[0]-(coordinates_prev[0]-0.0000000001)))
                        rotation = rotation -180
                        rotation1 = 10*math.atan((coordinates1[1]-coordinates_prev1[1]-0.0000000001)/(coordinates1[0]-(coordinates_prev1[0]-0.00000000001)))
                        ...
                        left_line=MapCreator._set_watch_points(distance=80,angle=line_rotation_left,location=coordinates,lat_change=lat_change,lon_change=lon_change,rotation=rotation)
                        right_line = MapCreator._set_watch_points(distance=80,angle=line_rotation_right,location=coordinates,lat_change=lat_change,lon_change=lon_change,rotation=rotation)
                        left_line2 = MapCreator._set_watch_points(distance=80,angle=line_rotation_left1,location=coordinates1,lat_change=lat_change1,lon_change=lon_change1,rotation=rotation1)
                        right_line2 =  MapCreator._set_watch_points(distance=80,angle=line_rotation_right1,location=coordinates1,lat_change=lat_change1,lon_change=lon_change1,rotation=rotation1)
                        fill_color = "blue"
                        ...
                        self.prev_drawing = 1
                        folium.PolyLine([coordinates_prev,coordinates],color=fill_color,weight=2).add_to(self.mapObj)
                        folium.PolyLine([coordinates,left_line],color=fill_color,weight=2).add_to(self.mapObj)
                        folium.PolyLine([coordinates,right_line],color=fill_color,weight=2).add_to(self.mapObj)
                        folium.PolyLine([coordinates_prev1,coordinates1],color=fill_color,weight=2).add_to(self.mapObj)
                        folium.Polygon(search_edges,color="blue",weight=2,fill=True,fill_color="orange",fill_opacity=0.4).add_to(self.mapObj)
       
            enemy_prev_list = enemy_list	
            return our_telemetry
    def create_polygon(self):
        ...
        try :
            folium.Polygon(edges).add_to(self.mapObj)
            folium.PolyLine([z_middle,points[0]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([z_middle,points[1]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([z_middle,points[2]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([z_middle,points[3]],color="red",weight = 1).add_to(self.mapObj)

            folium.PolyLine([q1_middle,side_points[0]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q1_middle,side_points[1]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q1_middle,side_points[2]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q1_middle,side_points[3]],color="red",weight = 1).add_to(self.mapObj)

            folium.PolyLine([q2_middle,side_points[4]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q2_middle,side_points[3]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q2_middle,side_points[5]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q2_middle,side_points[6]],color="red",weight = 1).add_to(self.mapObj)

            folium.PolyLine([q3_middle,side_points[5]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q3_middle,side_points[7]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q3_middle,side_points[8]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q3_middle,side_points[9]],color="red",weight = 1).add_to(self.mapObj)

            folium.PolyLine([q4_middle,side_points[2]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q4_middle,side_points[10]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q4_middle,side_points[11]],color="red",weight = 1).add_to(self.mapObj)
            folium.PolyLine([q4_middle,side_points[7]],color="red",weight = 1).add_to(self.mapObj)
